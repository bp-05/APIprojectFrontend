...
export type EmpresasPanelMode = 'admin' | 'vcm'
...
export default function EmpresasPanel({ mode }: { mode: EmpresasPanelMode }) {
            ...
            {loading ? (
              ...
              filtered.map((c) => (
                ...
                >
                  ...
                  <Td>
                    {isVCMView
                      ? renderResponsibleColumn(c, () => setAssigningContact(c))
                      : renderContactSummary(c.counterpart_contacts?.[0])}
                  </Td>
                  {isVCMView ? (
...
function renderContactSummary(contact?: CounterpartContact | null) {
  if (!contact) {
    return <span className="text-xs text-zinc-500">Sin responsable</span>
  }

  const name = contact.name?.trim()
  const role = contact.role?.trim() || contact.counterpart_area?.trim()
  const details = contact.email?.trim() || contact.phone?.trim()

  return (
    <div className="text-sm text-zinc-800">
      <div className="font-medium">{name || 'Sin nombre'}</div>
      <div className="text-xs text-zinc-600">{role || 'Sin cargo'}</div>
      <div className="text-xs text-zinc-600">{details || 'Sin datos de contacto'}</div>
    </div>
  )
}
