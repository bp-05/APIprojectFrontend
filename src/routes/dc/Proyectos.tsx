...
export default function DCProyectos() {
  ...
  async function load() {
    ...
    setError(null)
    try {
      const [ps, cs, ss] = await Promise.all([listProblemStatements(), listCompanies(), listSubjectCodeSections()])
      setItems(ps)
      setCompanies(cs)
      setSubjects(ss)
    } catch (e) {
  ...
  const filtered = useMemo(() => {
    ...
    return items.filter((p) =>
      [p.problem_to_address, p.why_important, p.stakeholders].filter(Boolean).some((v) => String(v).toLowerCase().includes(q))
    )
  }, [items, search])
...
function ProblemForm({
  ...
  async function onSubmit(e: React.FormEvent) {
    e.preventDefault()
    const payload = Object.fromEntries(Object.entries(form).map(([k, v]) => [k, typeof v === 'string' ? v.trim() : v])) as typeof form
    ...
    if (!payload.problem_to_address.trim()) {
      toast.error('Completa el problema a abordar')
      return
    }
    if (!payload.why_important.trim()) {
      toast.error('Completa por qué es importante')
      return
    }
    if (!payload.stakeholders.trim()) {
      toast.error('Completa los stakeholders')
      return
    }
    if (!payload.related_area.trim()) {
      toast.error('Completa el área relacionada')
      return
    }
    if (!payload.benefits_short_medium_long_term.trim()) {
      toast.error('Completa los beneficios')
      return
    }
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30 p-4">
      <div className="max-h-[calc(100vh-4rem)] w-full max-w-3xl overflow-y-auto rounded-xl bg-white shadow-lg ring-1 ring-black/5">
